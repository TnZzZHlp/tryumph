...
use rand::{
    distr::{Distribution, Uniform, uniform::Error as UniformError},
    random,
    rngs::ThreadRng,
};
...
/// Base duration is multiplied by a random number between 0.0 and 1.0.
...
mod tests {
    ...
    fn range_exclusive() {
        ...
        assert!(
            range.next().unwrap() >= Duration::from_millis(0)
                && range.next().unwrap() <= Duration::from_millis(1)
        );
        assert!(
            range.next().unwrap() >= Duration::from_millis(0)
                && range.next().unwrap() <= Duration::from_millis(1)
    ...
    #[test]
    fn range_inclusive() {
        let mut range = Range::from_millis_inclusive(0, 1);
        assert!(
            range.next().unwrap() >= Duration::from_millis(0)
                && range.next().unwrap() <= Duration::from_millis(1)
        );
